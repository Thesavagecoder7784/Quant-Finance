...
def create_database_schema(db_name: str = DATABASE_NAME):
...
def store_sentiment_data(data: list[dict], db_name: str = DATABASE_NAME):
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    for item in data:
...
def get_daily_aggregates(db_name: str = DATABASE_NAME) -> pd.DataFrame:
    conn = sqlite3.connect(db_name)
    query = """
        SELECT
