...
def run_backtesting_pipeline(start_date: str, end_date: str, description: str, is_optimization_run: bool = False, optimized_params: dict = None):
    ...
    If is_optimization_run is True, it performs parameter optimization and returns the best parameters.
    If optimized_params are provided, it uses them directly for the backtest.
    """
    ...
    if sentiment_df.empty:
        ...
        return None # Return None to indicate failure or no optimized params
    ...
    current_buy_threshold = config.BUY_THRESHOLD
    current_sell_threshold = config.SELL_THRESHOLD

    if is_optimization_run and optimized_params is None:
        # This is the training phase, perform optimization
        optimized_params = optimize_parameters(sentiment_df_train=sentiment_df, price_df_train=price_df)
        if optimized_params:
            current_buy_threshold = optimized_params['buy_threshold']
            current_sell_threshold = optimized_params['sell_threshold']
        else:
            logging.warning("Parameter optimization failed. Using default thresholds.")
    elif optimized_params:
        # This is the testing phase, use provided optimized parameters
        current_buy_threshold = optimized_params['buy_threshold']
        current_sell_threshold = optimized_params['sell_threshold']
    else:
        # No optimization requested, and no optimized params provided, use defaults
        logging.info("No optimization requested and no optimized parameters provided. Using default thresholds from config.py.")

    # Run the backtest with the determined parameters
    ...
    if is_optimization_run:
        return optimized_params # Return optimized parameters from the training run
    return None # For testing runs, no parameters are returned
''