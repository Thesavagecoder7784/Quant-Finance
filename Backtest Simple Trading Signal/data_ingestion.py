...
def fetch_newsapi(query: str, from_date: str, to_date: str) -> list[dict]:
    """
    Fetches financial headlines using NewsAPI.
    Queries by keywords (e.g., company names, financial terms).
    """
    articles = []
    url = f"https://newsapi.org/v2/everything?q={query}&from={from_date}&to={to_date}&language=en&sortBy=relevancy&apiKey={config.NEWSAPI_KEY}"
    try:
...
def fetch_reddit(subreddit: str, limit: int = 100) -> list[dict]:
    """
    Fetches financial headlines (titles of submissions) from Reddit using PRAW.
    """
...
def fetch_finviz(ticker: str) -> list[dict]:
    """
    Fetches news headlines for a specific ticker from Finviz using basic web scraping.
    This is a fragile method as website structure can change.
    """
...
def fetch_historical_prices(tickers: list[str], start_date: str, end_date: str) -> pd.DataFrame:
    """
    Fetches historical daily adjusted closing prices for a list of tickers using yfinance.
    """
    logging.info(f"Fetching historical prices for {tickers} from {start_date} to {end_date}.")
    try:
        data = yf.download(tickers, start=start_date, end=end_date)
        if 'Close' in data.columns and isinstance(data['Close'], pd.DataFrame):
            prices_df = data['Close']
        elif 'Close' in data.columns and isinstance(data['Close'], pd.Series):
             # For a single ticker, yf.download returns a Series for 'Close'
            prices_df = pd.DataFrame(data['Close'])
        else:
            # Fallback if 'Close' is not directly a DataFrame (e.g., for single ticker)
            prices_df = data['Close'].to_frame() if isinstance(data['Close'], pd.Series) else pd.DataFrame()

        prices_df.index = pd.to_datetime(prices_df.index).normalize()
        logging.info(f"Successfully fetched historical prices. Shape: {prices_df.shape}")
        return prices_df
    except Exception as e:
